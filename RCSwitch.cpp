...
unsigned int RCSwitch::timings[RCWire::frameSize * 2 + 3];
unsigned char RCSwitch::nReceivedCode[RCWire::frameSize];

RCSwitch::RCSwitch() {
    this->nTransmitterPin = -1;
    this->setRepeatTransmit(20);  //raised to 20 transmissions without ACK
    this->setProtocol(1);
...
/**
 *
 * Modified by Gstöttner Robert
 */
void RCSwitch::send(unsigned char *code, int length) {
    isSending = true;  //added to prevent receiving own messages on raspberry
    if (this->nTransmitterPin == -1)
        return;
    ...
    isSending = false; //added to prevent receiving own messages on raspberry
}
...
bool RECEIVE_ATTR RCSwitch::receiveProtocol(const int p, unsigned int changeCount) {
    ...
    if (changeCount >
        7) {    // ignore very short transmissions: no device sends them, so this must be noise
...
bool RECEIVE_ATTR RCSwitch::receiveProtocolInCodeArrayMode(const int p, unsigned int changeCount) {
#ifdef ESP8266
    const Protocol &pro = proto[p-1];
#else
    Protocol pro;
    ...
    for (unsigned int i = firstDataTiming; i < changeCount - 1; i += 2) {
        if (diff(RCSwitch::timings[i], delay * pro.zero.high) < delayTolerance &&
            diff(RCSwitch::timings[i + 1], delay * pro.zero.low) < delayTolerance) {
            nReceivedCode[index] = 0;
            //zero
        } else if (diff(RCSwitch::timings[i], delay * pro.one.high) < delayTolerance &&
                   diff(RCSwitch::timings[i + 1], delay * pro.one.low) < delayTolerance) {
            // one
            nReceivedCode[index] = 1;
        } else {
            // Failed
            return false;
        }
...
void RECEIVE_ATTR RCSwitch::handleInterrupt() {
    if (isSending) { //added to prevent receiving own messages on raspberry
        return;
    }
    ...
    // detect overflow
    if (changeCount >= (RCWire::frameSize * 2 + 3)) {
        changeCount = 0;
        repeatCount = 0;
    }
